...
theorem sim_step_ret_forwardc_sink
  ...
  { proc := pc.proc,
    chans := intermChans ec pc vars
      (retValsToExprOutputs (instInterp := instInterp) retVals)
      ec.pathConds
    state := pc.state }
:= by
  have ⟨
  ...
  have ⟨chans₁, retVals', hpop_ret_vals, hchans₁, hret_vals⟩ :=
    pop_vals_singleton _ _
    (map := pc.chans)
    (names := compileExpr.varNames χ ec.pathConds vars)
    (λ name val =>
      ∃ var,
    ...
    (by
      simp [hsim.vars_to_chans, compileExpr.varNames,
  ...
  have hmem_forwardc :
    AtomicProc.forwardc
      (compileExpr.varNames χ ec.pathConds vars)
      ((Vector.replicate m (Interp.junkVal (self := instInterp) Op S)).push
        (Interp.fromBool (self := instInterp) Op S false))
  ...
  · intros name hmem_name
    simp at hmem_name
    simp [hchans₁, compileExpr.varNames,
  ...
  have ⟨chans₂, otherVals, hpop_other_vals, hchans₂, hother_vals⟩ :=
    ...
    (by
      ...
      simp [h, compileExpr.varNames, compileExpr.varName, hnot_mem_var])
  have hmem_sink :
...
theorem sim_step_ret_exec_dataflow
  ...
  (hexpr : ec.expr = .cont (.ret vars))
  (hpc_atoms : pc.proc.atoms = compileFn.initCarry Op χ V ec.fn true :: rest) :
  Dataflow.Config.StepPlus Op (ChanName χ) V S pc {
    proc := {
      inputs := pc.proc.inputs,
      outputs := pc.proc.outputs,
      atoms := compileFn.initCarry Op χ V ec.fn false :: rest
    },
