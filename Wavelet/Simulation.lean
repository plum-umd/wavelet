...
theorem pop_val_singleton_rewrite
  {map : ChanMap χ V}
  (hsingleton : map name = [val]) :
  map.popVal _ _ name = some (val, λ n => if n = name then [] else map n) := sorry
...
theorem pop_vals_singleton_rewrite
  ...
  (hsingletons : ∀ name ∈ names, ∃ val, map name = [val] ∧ prop name val) :
  ∃ vals,
    map.popVals _ _ names = some (vals, λ n => if n ∈ names then [] else map n) ∧
    List.Forall₂ prop names.toList vals.toList := sorry
...
-- theorem sim_step_br_chans₃_empty_var
--   {ec : Seq.Config Op χ V S m n} {cond}
--   {chans chans₁ chans₂ chans₃ : ChanMap (ChanName χ) V}
--   {inputVals : Vector V _}
--   (hchans : chans = SimR.varsToChans _ _ _ _ ec)
--   (hchans₁ : chans.popVal _ _ condName = some (condVal, chans₁))
--   (hchans₂ : (chans₁.pushVals _ _
--     #v[.switch_cond condName, .merge_cond condName]
--     (Vector.replicate 2 condVal)).popVal _ _ (.switch_cond condName) = some (condVal, chans₂))
--   (hchans₃ : chans₂.popVals _ _
--     (compileExpr.allVarsExcept χ ec.definedVars cond ec.pathConds) = some (inputVals, chans₃))
...
theorem sim_step_br
  ...
  | step_br hexpr hcond =>
    ...
    := by
      ...
      simp [← hpc₁]
    -- Step 2: Fire the `switch` operator
    have hmem_switch :
    ...
    := by
      simp [← hpc₁]
      apply pop_val_singleton
      rw [push_vals_lookup_singleton _ _ 0]
      · simp
        rw [pop_val_lookup_diff _ _ hchans₁]
    ...
    := by
      ...
      if heq : ec.pathConds = pathConds then
        if hmem_var : var ∈ ec.definedVars.erase cond then
          ...
          rw [pop_vals_lookup_singleton _ _ hchans₃
            (by simp [← heq, hmem_var, compileExpr.allVarsExcept, List.toVector])]
          rw [pop_val_lookup_diff _ _ hchans₂ (by simp)]
          simp [← hpc₁]
          rw [push_vals_lookup_diff _ _ (by simp)]
          rw [pop_val_lookup_diff _ _ hchans₁]
          ...
          rw [pop_vals_lookup_diff _ _ hchans₃]
          · rw [pop_val_lookup_diff _ _ hchans₂ (by simp)]
            simp [← hpc₁]
            rw [push_vals_lookup_diff _ _ (by simp)]
            if heq_cond : cond = var then
              simp only [← heq, ← heq_cond, hcondName]
              rw [pop_val_lookup_singleton _ _ hchans₁]
              simp [hlive_vars, SimR.varsToChans, ← hcondName, hcond]
            else
              rw [pop_val_lookup_diff _ _ hchans₁ (by simp [← hcondName, heq_cond])]
              simp [hlive_vars, SimR.varsToChans, heq]
              split
              · rename_i h
                have := (hdefined_vars var).mpr ⟨_, h⟩
                have := (List.mem_erase_of_ne (Ne.symm heq_cond)).mpr this
                contradiction
              · rfl
    ...
    · funext name
      ...
      | var v pathConds =>
        ...
        · rename_i hpath_conds
          if hmem_v : v ∈ ec.definedVars.erase cond then
            ...
            simp
            rw [push_vals_lookup_singleton _ _ i (val := val)]
      ...
      | merge_cond name =>
        ...
        · simp only [← hcondName] at hname
          ...
          · rw [pop_vals_lookup_diff _ _ hchans₃]
            · rw [pop_val_lookup_diff _ _ hchans₂]
              · simp only [← hpc₁]
                rw [push_vals_lookup_singleton _ _ 1]
                · simp
                  rw [pop_val_lookup_diff _ _ hchans₁]
                  · simp only [hlive_vars, SimR.varsToChans]
                    split
      ...
      | switch_cond v =>
        simp
        rw [push_vals_lookup_diff _ _ (by simp [compileExpr.allVarsExcept])]
        rw [pop_vals_lookup_diff _ _ hchans₃ (by simp [compileExpr.allVarsExcept])]
      ...
      | _ =>
        ...
        rw [push_vals_lookup_diff _ _ (by simp)]
        rw [pop_val_lookup_diff _ _ hchans₁]
        · simp [hlive_vars, SimR.varsToChans, hbr]
        · simp [← hcondName]
    · exact List.Nodup.erase _ hdefined_vars_no_dup
    · grind [VarMap.getVar, VarMap.removeVar]
    · grind
    · grind
    · simp only [← hpc₂, hmerges]
